...
void heapify(int *dst, int *src, int n) {
    /* If so, I will gladly use it over there */
    void
    heap_it( int p, int i, int* out, int size, int* in ) {
        if( p >= size ) return;

        out[ p ] = in[ i ];
        heap_it( p * 2 + 1, i / 2, out, size, in);
        heap_it( p * 2 + 2, i / 2, out, size, & in[ i + 1 ]);
    }

    heap_it( 0, n / 2, dst, n, src );
}
