...
fn draw_map( state: & State, map: & Map ) {
    for (y, line) in map.iter().enumerate() {
        let mut line = line.clone();
        if state.guy.1 as usize == y { line[ state.guy.0 as usize ] = 'K'; }
        for &(x,_) in state.diamonds.iter().filter( |&&(_, yy)| y == yy as usize ) {
            line[ x as usize ] = 'D';
        }
...
fn astar_heuristic( next: & State, mut areas: Areas ) -> usize {
    let mut value = 0;
...
fn astar( base: State, map: Map, areas: Areas ) -> Option<String> {
    let mut queue = BinaryHeap::new();
    let mut states = HashMap::new();
    let mut costs = HashMap::new();

    queue.push( Item::new( base.clone(), 0 ) );
    states.insert( base.clone(), None::<(State, Move)> );
    costs.insert( base, 0 );
    ...
    while let Some( state ) = queue.pop() {
        ...
        if areas.is_superset( & state.diamonds.iter().cloned().collect() ) {
            ...
            while let Some( saved ) = query.as_ref() {
                moves.push_front( saved.1.as_char() );
                query = states.get( & saved.0 ).unwrap();
            }

            return Some( moves.iter().collect() );
        }

        let cost = costs.get( & state ).unwrap() + 1;

        for (new_state, movement) in generate_moves( & state, & map ) {
            match costs.entry( new_state.clone() ) {
                Entry::Occupied( mut entry ) => {
                    if cost < *entry.get() {
                        *entry.get_mut() = cost;
                        *states.get_mut( & new_state ).unwrap() = Some( (state.clone(), movement) );

                        let priority = cost + astar_heuristic( & new_state, areas.clone() );
                        queue.push( Item::new( new_state, priority ) );
                    }
                },
                Entry::Vacant( entry ) => {
                    entry.insert( cost );
                    states.insert( new_state.clone(), Some( (state.clone(), movement) ) );
...
fn bfs( base: State, map: Map, areas: Areas ) -> Option<String> {
    let mut queue = VecDeque::new();
    let mut states = HashMap::new();

    queue.push_back( base );
    states.insert( queue[ 0 ].clone(), None::<(State, Move)> );
    ...
    while let Some( state ) = queue.pop_front() {
        ...
        for (new_state, movement) in generate_moves( & state, & map ) {
            ...
            states.insert( new_state, Some((state.clone(), movement)) );
        }
    }
...
fn application() -> io::Result<()> {
    ...
    let result = astar( state, map, areas );
