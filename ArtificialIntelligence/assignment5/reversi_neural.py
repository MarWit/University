...
with open('/home/marwit/Downloads/reversi_learning_data/smaller.dat' ) as f:
...
model.fit( games, llabels, epochs = 5, batch_size = 128 )
...
def neural_result( board, player ):
    ...
    b = np.array( b ).reshape( (1, 192) )
    v = model.predict( b )

    return int( v[ 0 ][ 0 ] > v[ 0 ][ 1 ] )
