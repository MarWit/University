...
(define (eval-let exp env)
    (let ([bindings (bindings-to-list (let-bindings exp) env)])
        (apply-proc
            (make-procedure (car bindings) (let-body exp) env)
            (cdr bindings)
        )
    )
)
...
(define (let*? exp) (tagged-list? exp 'let*))
(define (eval-let* exp env)
    (eval (car (curry-bindings (let-bindings exp) (let-body exp))) env)
)

(define (curry-bindings bindings body)
    (if (empty? bindings) body
        (let ([var (caar bindings)]
              [value (cadar bindings)])
            (list (make-application
                (make-lambda (list var) (curry-bindings (cdr bindings) body))
                (list value)
            ))
        )
    )
...
(define (bindings-to-list bindings env)
    (if (empty? bindings) (cons '() '())
        (let ([var (caar bindings)]
              [value (eval (cadar bindings) env)]
              [rest (bindings-to-list (cdr bindings) env)])
            (cons (cons var (car rest)) (cons value (cdr rest)))
        )
    )
)
...
(define primitive-procedures
  (list (list 'car car)
        ...
        (list '* *)
        (list '= =)
        (list 'print print))) ; add more primitive procedures here
